# nf-core/chrmtags: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/chrmtags/usage](https://nf-co.re/chrmtags/usage)
...
```console
nextflow run nf-core/chrmtags --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/chrmtags
```
...
First, go to the [nf-core/chrmtags releases page](https://github.com/nf-core/chrmtags/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
