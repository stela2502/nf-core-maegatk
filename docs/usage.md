# nf-core/mitotags: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/mitotags/usage](https://nf-co.re/mitotags/usage)
...
```console
nextflow run nf-core/mitotags --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/mitotags
```
...
First, go to the [nf-core/mitotags releases page](https://github.com/nf-core/mitotags/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
